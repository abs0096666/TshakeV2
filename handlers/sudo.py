	...
	if text and (type is enums.ChatType.PRIVATE or (type is enums.ChatType.SUPERGROUP or type is enums.ChatType.GROUP)) :
		...
		if re.search(c.STreplyBOT, text):
			...
			if redis.hexists("{}Nbot:TXreplys".format(BOT_ID,chatID),tx):
				...
				kb = InlineKeyboardMarkup([[InlineKeyboardButton(r.MoreInfo, url="t.me/homespeople2")]])
		...
		if rank is "sudo" or rank is "asudo":
			...
			if text == c.ADDfiles:
				url = "https://raw.githubusercontent.com/abs0096666/TshakeV2-files/master/files"
			...
			if text == c.Ubot:
				...
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/abs0096666/TshakeV2/master/handlers/"+fnh
					out = requests.get(url).text
					f = open("./handlers/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/abs0096666/TshakeV2/master/utlis/"+fnu
					out = requests.get(url).text
					f = open("./utlis/"+fnu,"w+")
					f.write(out)
					f.close()
				for fnb in Files_B:
					url = "https://raw.githubusercontent.com/abs0096666/TshakeV2/master/"+fnb
					out = requests.get(url).text
					f = open("./"+fnb,"w+")
					f.write(out)
					f.close()
				for fnu in Files_L:
					url = "https://raw.githubusercontent.com/abs0096666/TshakeV2/master/lang/"+fnu
					out = requests.get(url).text
					f = open("./lang/"+fnu,"w+")
					f.write(out)
					f.close()
